  ...
  return (
    <div className="flex justify-center w-full h-24 p-8 bg-transparent navbar">
      ...
      <div className="xl:w-[5%] lg:w-[15%] text-center hidden lg:flex justify-end xl:justify-center">
        {
          account ? (<FaRegUser className="border w-10 h-10 rounded-[50%] p-2" onClick={handleLogoutPop}/>
          ): <button className="wallet-popup w-full max-w-[100px] flex justify-center items-center text-black font-bold rounded-full p-2" onClick={handleWalletToggle}>Wallet</button>
        }
     
      </div>
      {
        walletPopUp && <div className="fixed top-0 left-0 bottom-0 right-0 walletpop-overlay z-[150] flex justify-center">
        <div className="w-[90%] relative max-w-[900px] h-[500px]  my-48 bg-white text-black">
          <button className="absolute right-5 top-4 font-bold" onClick={handleWalletToggle}>X</button>
          <div className="flex flex-col items-center justify-center h-[400px]">
            <h1 className="capitalize font-bold text-2xl m-4">service providers</h1>
            <div className="flex flex-col sm:flex-row w-[60%] h-48 sm:w-full max-w-[500px] justify-between items-center">
              <div className="metamask-icon" onClick={()=>{onConnectToMetamaskFunc();handleWalletToggle()}}></div>
              <div className="wallet-connet" onClick={()=>onConnectWithWalletConnect()}></div>
            </div>
          </div>
        </div>
      </div>
       ...
       {
        logOutPopup && <div className="fixed top-0 left-0 bottom-0 right-0 walletpop-overlay z-[150] flex justify-center">
        <div className="w-[90%] relative max-w-[300px] h-[300px]  my-48 bg-white text-black rounded-lg">
          <button className="absolute right-5 top-4 font-bold " onClick={handleLogoutPop}>X</button>
          <div className="flex flex-col items-center justify-center h-[300px] lowercase">
            <button className="wallet-popup w-full max-w-[100px] flex justify-center items-center text-black font-bold rounded-full p-2 my-2" 
            onClick={()=>{logout(); handleLogoutPop()}}
            >Logout</button>
